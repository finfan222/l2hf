...
import org.lineage.gameserver.taskmanager.RandomAnimationTaskManager;
import org.lineage.gameserver.util.UnboundArrayList;
		...
		if (!isOn)
		{
			for (int i = 0; i < _visibleObjects.size(); i++)
			{
				...
				if (wo.isAttackable())
				{
					...
					if (mob.hasAI())
					{
						mob.getAI().setIntention(org.lineage.gameserver.ai.CtrlIntention.AI_INTENTION_IDLE);
