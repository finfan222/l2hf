 ...
 */
package org.lineage.gameserver.instancemanager.games;
...
import org.lineage.gameserver.model.zone.type.DerbyTrackZone;
import org.lineage.gameserver.network.SystemMessageId;
import org.lineage.gameserver.network.serverpackets.DeleteObject;
import org.lineage.gameserver.network.serverpackets.MonRaceInfo;
	...
	public void newRace()
	{
		...
		for (int i = 0; i < 8; i++)
		{
			try
			{
				...
				_monsters[i] = (Npc) Class.forName("org.lineage.gameserver.model.actor.instance." + template.getType() + "Instance").getConstructors()[0].newInstance(template);
			}
			catch (Exception e)
			{
